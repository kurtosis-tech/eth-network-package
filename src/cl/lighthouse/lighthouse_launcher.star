...
def get_beacon_config(
	...
	if boot_cl_client_ctxs != None:
		cmd.append("--boot-nodes="+",".join([ctx.enr for ctx in boot_cl_client_ctxs[package.io_MAX_ENR_ENTRIES]]))
		cmd.append("--trusted-peers="+",".join([ctx.peer_id for ctx in boot_cl_client_ctxs[package.io_MAX_ENR_ENTRIES]]))
