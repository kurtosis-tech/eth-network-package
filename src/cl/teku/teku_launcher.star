...
def get_config(
	...
	if bootnode_contexts != None:
		cmd.append("--p2p-discovery-bootnodes="+",".join([ctx.enr for ctx[:20] in bootnode_contexts]))
		cmd.append("--p2p-static-peers="+",".join([ctx.multiaddr for ctx[:20] in bootnode_contexts]))
