...
def get_config(
	...
	if bootnode_contexts != None:
		cmd.append("--p2p-discovery-bootnodes="+",".join([ctx.enr for ctx in bootnode_contexts[:20]]))
		cmd.append("--p2p-static-peers="+",".join([ctx.multiaddr for ctx in bootnode_contexts[:20]]))
